...
- 前端进度页依赖轮询 status，日志为本地拼接，不是后端真实 task_logs。
...
- TaskStatusResult 扩展：
  ...
  - currentStepProgress
- 所有 step 统一写入 task_steps（running/finished/failed + timestamps + retry_count）。

### 3.3 可靠性与恢复
- 工作目录改为配置项（避免硬编码 /tmp）：
  - `smartark.agent.workspace-root`
- 打包、文件落盘、模型失败增加错误分类（MODEL_ERROR/IO_ERROR/VALIDATION_ERROR）。
...
- TaskProgressPage 去除 projectId 占位逻辑，使用后端返回 projectId。
