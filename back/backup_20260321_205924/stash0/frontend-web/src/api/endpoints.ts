...
export const taskApi = {
  ...
  releasePreview: (taskId: string): Promise<TaskPreviewResult> =>
    requestJson<TaskPreviewResult>({ method: 'POST', url: `/api/task/${taskId}/preview/release` }),
  relations: (sourceTaskId: string): Promise<TaskRelationChainResult> =>
    requestJson<TaskRelationChainResult>({ method: 'GET', url: `/api/task/${sourceTaskId}/relations` }),
  ...
  modifyFromPlan: (sourceTaskId: string, payload: { changeInstructions: string; iterationDoc?: string; chatSessionId?: string }) =>
    requestJson<GenerateResult>({
      url: `/api/task/${sourceTaskId}/modify/from-plan`,
      method: 'POST',
      data: payload
    }),
