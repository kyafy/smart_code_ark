...
export const handleMock = async (req: MockRequest): Promise<ApiResponse<unknown> | Blob> => {
  ...
  if (method === 'POST' && url.startsWith('/api/task/') && url.endsWith('/preview/release')) {
    const taskId = url.replace('/api/task/', '').replace('/preview/release', '')
    const data = getTask(taskId)
    if (!data) return { code: 1004, message: 'task not found', data: null }
    const task = data.task
    task.previewStatus = 'expired'
    task.previewPollCount = 0
    task.previewExpireAt = new Date().toISOString()
    task.previewLastError = '预览资源已手动释放'
    task.logs.push({ level: 'info', content: '预览资源已手动释放', ts: Date.now() })
    const r: TaskPreviewResult = {
      taskId,
      status: 'expired',
      previewUrl: null,
      expireAt: task.previewExpireAt,
      lastError: task.previewLastError
    }
    setState(data.state)
    return ok(r)
  }
  ...
  if (method === 'POST' && url.startsWith('/api/task/') && url.endsWith('/modify/from-plan')) {
    const body = (req.body ?? {}) as Record<string, unknown>
    const taskId = url.replace('/api/task/', '').replace('/modify/from-plan', '')
    const src = getTask(taskId)
    if (!src) return { code: 1004, message: 'task not found', data: null }
    const newTaskId = genId('t')
    const task: Task = {
      ...
      progress: 0,
      step: 'queued',
      logs: [{ level: 'info', content: '迭代编排任务已入队', ts: Date.now() }],
      previewStatus: 'provisioning',
      previewPollCount: 0,
      previewExpireAt: null,
      previewLastError: null,
    ...
    state.tasks.unshift(task)
    state.taskRelations.unshift({
      parentTaskId: taskId,
      childTaskId: newTaskId,
      relationType: 'modify_from_plan',
  ...
  if (method === 'GET' && url.startsWith('/api/task/') && url.endsWith('/relations')) {
    const sourceTaskId = url.replace('/api/task/', '').replace('/relations', '')
    const src = getTask(sourceTaskId)
    if (!src) return { code: 1004, message: 'task not found', data: null }
    const relations = state.taskRelations
      .filter((r) => r.parentTaskId === sourceTaskId)
      .map((r) => {
        const t = state.tasks.find((it) => it.taskId === r.childTaskId)
        return {
    ...
    return ok({ sourceTaskId, relations })
  }
