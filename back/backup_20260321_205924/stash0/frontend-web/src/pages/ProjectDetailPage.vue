...
const latestTaskId = computed(() => project.value?.tasks?.[0]?.id || '')
      ...
      <div class="bg-surface rounded-xl p-6 shadow-sm border border-border">
        ...
        <div class="mt-4">
          <el-button
            ...
            @click="latestTaskId && onProgress(latestTaskId)"
          >
            进入编排页
          </el-button>
        </div>
              ...
              <div v-for="task in project.tasks" :key="task.id" 
                ...
                <div class="flex gap-2">
                  ...
                  <template v-if="task.status === 'finished'">
                    <el-button type="primary" plain size="small" @click="onProgress(task.id)">
                      继续编排
                    </el-button>
