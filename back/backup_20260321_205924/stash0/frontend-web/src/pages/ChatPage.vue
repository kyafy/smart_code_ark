...
import { chatApi, taskApi } from '@/api/endpoints'
...
const modifyPlanMode = computed(() => String(route.query.mode ?? '') === 'modify_plan')
const modifySourceTaskId = ref('')
const modifyProjectId = ref('')
...
const canConfirm = computed(() => {
  if (modifyPlanMode.value) {
    return Boolean(modifySourceTaskId.value && lastAssistant.value?.message)
  }
  return chat.hasDraft
})

const modifyPlanDoc = computed(() => {
  if (!modifyPlanMode.value) return ''
  return lastAssistant.value?.message?.trim() || ''
})

const buildModifyPlanPrompt = (suggestion: string) => {
  return [
    '请基于以下用户修改建议，输出“迭代文档”。',
    '要求用中文，结构必须包含：',
    '1) 迭代目标',
...
const extractModifyContext = () => {
  if (!modifyPlanMode.value) {
    modifySourceTaskId.value = ''
    modifyProjectId.value = ''
    modifySuggestion.value = ''
    return
  }
  modifySourceTaskId.value = String(route.query.sourceTaskId ?? '').trim()
  modifyProjectId.value = String(route.query.projectId ?? '').trim()
  modifySuggestion.value = String(route.query.initialMessage ?? '').trim()
}

onMounted(async () => {
  if (!sessionId.value) return
  extractModifyContext()
  ...
  if (sessionId.value === 'new') {
    ...
    if (initialMessage) {
      ...
      await onSend(modifyPlanMode.value ? buildModifyPlanPrompt(initialMessage) : initialMessage)
    }
    return
  }
...
const onSend = async (text: string) => {
  try {
    if (sessionId.value === 'new') {
      ...
      const nextQuery: Record<string, string> = { initialMessage: text }
      if (modifyPlanMode.value) {
        nextQuery.mode = 'modify_plan'
        if (modifySourceTaskId.value) nextQuery.sourceTaskId = modifySourceTaskId.value
        if (modifyProjectId.value) nextQuery.projectId = modifyProjectId.value
      }
      await router.replace({ name: 'chat', params: { sessionId: newSid }, query: nextQuery })
      return
    }
...
const onConfirm = async () => {
  ...
  if (modifyPlanMode.value) {
    if (!modifySourceTaskId.value) {
      ElMessage.warning('缺少来源任务ID，无法创建修改任务')
      return
    }
    const instructions = (modifySuggestion.value || '').trim()
    if (!instructions) {
      ElMessage.warning('缺少修改建议，无法创建修改任务')
      return
    }
    const iterationDoc = modifyPlanDoc.value
    isConfirming.value = true
    try {
      const res = await taskApi.modifyFromPlan(modifySourceTaskId.value, {
        changeInstructions: instructions,
        iterationDoc,
        chatSessionId: chat.sessionId || ''
      })
      ElMessage.success('已创建修改任务，正在进入编排页')
    ...
    <div class="col-span-12 lg:col-span-6">
      <div v-if="sessionId === 'new'" class="flex h-full flex-col items-center justify-center text-center py-20">
        ...
        <div class="mt-2 max-w-xl text-sm text-slate-500 dark:text-slate-400">
          {{ modifyPlanMode ? '在下方输入框补充修改细节，AI 将先生成迭代文档并确认编排。' : '在下方输入框描述你的需求，AI 将为你梳理功能并生成项目' }}
        </div>
      </div>
      ...
      <div v-else class="rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-900 dark:bg-slate-950">
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <el-button size="small" type="primary" :loading="isConfirming" :disabled="!canConfirm" @click="onConfirm">
              {{ modifyPlanMode ? '确认编排' : '生成项目' }}
            </el-button>
          </div>
        </div>

        <div v-if="!modifyPlanMode" class="mt-4 flex flex-wrap items-center gap-2">
          <button
            type="button"
        ...
        <div class="mt-4 rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-900 dark:bg-slate-950">
          <div v-if="!modifyPlanMode && activeStep === 1" class="text-sm leading-relaxed">
          ...
          <div v-else-if="modifyPlanMode || activeStep === 2" class="text-sm leading-relaxed">
            <div class="text-sm font-semibold mb-3">{{ modifyPlanMode ? 'AI 迭代文档' : 'AI确认需求' }}</div>
    ...
    <div class="col-span-12 lg:col-span-3 lg:self-start">
      <div v-if="sessionId !== 'new'" class="rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-900 dark:bg-slate-950 lg:sticky lg:top-6">
        <div class="flex items-start justify-between gap-3">
          <div>
            <div class="text-sm font-semibold">{{ modifyPlanMode ? '迭代文档' : '需求文档' }}</div>
            <div class="mt-1">
          ...
          <el-button size="small" type="success" :loading="isConfirming" @click="onConfirm" :disabled="!canConfirm">
            {{ modifyPlanMode ? '确认编排' : '启动生成' }}
          </el-button>
        </div>

        <div class="mt-4 space-y-4">
          <div class="rounded-2xl border bg-slate-50 p-4 dark:border-slate-900 dark:bg-slate-900/40">
            <div class="text-xs font-semibold text-slate-700 dark:text-slate-200">{{ modifyPlanMode ? '迭代概览' : '项目概览' }}</div>
            <div class="mt-2">
              <MarkdownRenderer :content="modifyPlanMode ? (modifyPlanDoc || overviewText) : overviewText" />
            </div>
          </div>

          <div v-if="!modifyPlanMode && chat.extractedRequirements?.features?.length" class="rounded-2xl border bg-slate-50 p-4 dark:border-slate-900 dark:bg-slate-900/40">
          ...
          <div v-if="!modifyPlanMode && chat.extractedRequirements?.pages?.length" class="rounded-2xl border bg-slate-50 p-4 dark:border-slate-900 dark:bg-slate-900/40">
          ...
          <div v-if="!modifyPlanMode && !chat.extractedRequirements" class="rounded-2xl border bg-slate-50 p-4 dark:border-slate-900 dark:bg-slate-900/40">
