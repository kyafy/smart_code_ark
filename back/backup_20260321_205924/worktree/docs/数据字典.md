...
| 字段 | 来源 | 说明 |
| --- | --- | --- |
| `X-Client-Platform` | 请求头 | 客户端平台：`web/miniprogram/app` |
| `X-App-Version` | 请求头 | 客户端版本号 |
| `X-Device-Id` | 请求头 | 设备唯一标识（匿名） |
| `traceId` | `X-Trace-Id` | 全链路请求追踪 ID |

说明：

1. 以上字段在服务端通过请求上下文透传到日志，不直接落库到业务表。
2. 后续如需端维度运营分析，可再扩展专用审计/行为表。
